...
server.tool(
  ...
  async ({ action, jobId, from, to, category, counties, includeOther, cases, label, maxCases, directory, rankId, skipGroupC, useMirror }) => {
    const reply = (text: string) => ({ content: [{ type: "text" as const, text }] });
    try {
      if (action === "list") {
